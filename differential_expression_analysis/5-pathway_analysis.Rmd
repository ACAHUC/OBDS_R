...
all_genes_df <- read_csv("/cloud/project/differential_expression_analysis/CD8_vs_CD4_DE_results_lfcshrinkage_allgenes.csv")
dim(all_genes_df)
...
downreg_genes_df <- upreg_genes_df <- all_genes_tested_df %>%
    filter(padj < 0.05 & log2FoldChange < -1)
...
gost_result <- gost(upreg_gene_ids, organism = "mmusculus", correction_method = "fdr") 

# write query and assign to output variable named `gost_results_obj`
gost_results_obj <- gost(upreg_gene_ids, organism = "mmusculus", correction_method = "fdr")
