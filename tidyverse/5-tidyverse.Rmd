...
```{r}
iris_raw <- read_csv("/cloud/project/data/tidyverse/iris.csv")
```
...
> Answer:A tibble, 150 x 5, dbl and chr
...
iris_raw %>% 
  group_by(species) %>%
summarise(sepal_length.mean = mean(sepal_length), sepal_length.median = median(sepal_length), sepal_length.minimum = min(sepal_length), sepal_length.maximum = max(sepal_length))
...
iris_raw %>% 
    group_by(species) %>%
  summarise(dplyr::across(tidyselect::where(is.numeric), mean))
```
...
```{r}
iris_long <- iris_raw %>% 
  pivot_longer(cols=c(sepal_length, sepal_width, petal_length, petal_width),names_to='Variable')
...
iris_long2 <- iris_raw %>% 
   mutate(ID = as.character(seq(1,nrow(iris_raw)))) %>%  
 pivot_longer(cols=c(sepal_length, sepal_width, petal_length, petal_width),names_to='Variable')
...
```{r}
library(readxl)
excel_sheets("/cloud/project/tidyverse/EH2011.xlsx") #this load the whole excel file
exprs_assay <- read_excel("/cloud/project/tidyverse/EH2011.xlsx","exprs") #this assign the specific excel sheet called exprs
...
exprs_assay_long <- exprs_assay %>% 
  pivot_longer(cols=c(where(is.numeric)),names_to='Sample')
...
```{r}
excel_sheets("/cloud/project/tidyverse/EH2011.xlsx")
sample_info <- read_excel("/cloud/project/tidyverse/EH2011.xlsx", "sample_info")
sample_info
```
