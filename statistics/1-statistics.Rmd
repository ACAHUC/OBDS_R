...
```{r}
z <- quantile(y, probs = seq(0, 1, 0.10), na.rm = FALSE)
z
df_y <- data.frame(y)
```
...
ggplot(df_y) +
  geom_histogram(aes(y), binwidth=1, colour = "white")
...
ggplot(df_y) +
  geom_histogram(aes(y), binwidth=1, colour = "white") +
  geom_vline(xintercept = c(mean_y, median_y, mean_y + sd_y, mean_y - sd_y, median_y + mad_y, median_y - mad_y),
...
```{r}
set.seed(1)
lot <- rnorm(n = 1e6, mean = 10, sd = 5)
df_lot <- data.frame(lot)
mean_lot = mean(lot)
median_lot = median(lot)
sd_lot = sd(lot)
mad_lot = mad(lot)
ggplot(df_lot) +
  geom_histogram(aes(lot), binwidth=1, colour = "white") +
  geom_vline(xintercept = c(mean_y, median_y, mean_y + sd_y, mean_y - sd_y, median_y + mad_y, median_y - mad_y),
    colour = c("red", "red", "blue", "blue", "blue", "blue"),
    linetype = c(1, 2, 1, 1, 2, 2))
...
q <- qnorm(y, mean = 0, sd = 1, lower.tail = TRUE, log.p =FALSE)
plot(x = y, y = q)
...
```{r}
1-pnorm(2)
#opposite of pnorm (consider 1 as 100%)
```

- What is the probability of observing a value between -2 and 2?

```{r}
l <- pnorm(-2) #calculate -2 and less
r <- pnorm(2) #calculate 2 and less
n <- r-l # divide the value below -2
n
```
...
#excercise 
#Run a Wilcoxon test to compare the vectors
1:10 and 5:14 .
What is the p-value?
...
Run a t-test to compare the gene expression
values between the two groups.
What is the p-value?
```{r}
gene_data <- read_csv("/cloud/project/data/statistics/gene_exprs.csv")
t.test(gene_exprs ~ group, data=gene_data)
```

```{r}
t.test(
  x = gene_data$gene_exprs[gene_data$group == "groupA"],
  y = gene_data$gene_exprs[gene_data$group == "groupB"]
)
```
Excercise
Testing & Multipletestingcorrection
Given an Excel file that contains a matrix of log-normalised counts ( logcounts ) and experimental
metadata ( cell_info ), test each gene (i.e., row) in the matrix for differential expression between the two
experimental groups. 
...
```{r}
excel_sheets("/cloud/project/data/statistics/GSE111543.xlsx")
summary(logcounts_matrix_table)

logcounts_matrix_table <- read_excel("/cloud/project/data/statistics/GSE111543.xlsx", "logcounts")

logcounts_matrix_table_1 <- as.data.frame(logcounts_matrix_table)

logcounts_matrix_table_2 <- column_to_rownames(logcounts_matrix_table_1, var = "gene")

dim (logcounts_matrix_table_1)

cell_info <- read_excel("/cloud/project/data/statistics/GSE111543.xlsx", "cell_info")

unique(cell_info$Infection) # to check the content inside column
...
expression_infected <- logcounts_matrix_table_2[1, cells_infected]
expression_not_infected <- logcounts_matrix_table_2[1, cells_not_infected]
...
```
```{r}
gene_names <- (rownames(logcounts_matrix_table_2)) 

t_test_pvalues <- vapply ( X = gene_names, FUN = t_test_row, FUN.VALUE = numeric(1))
