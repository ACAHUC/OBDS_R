...
```{r}
truseq_adapters <- readDNAStringSet("/cloud/project/data/bioconductor/TruSeq3-PE-2.fa")
truseq_adapters
str(truseq_adapters)
```
...
```{r}
gtf_data <- rtracklayer::import( "/cloud/project/data/bioconductor/Homo_sapiens.GRCh38.104.gtf.gz"  )
gtf_data
```
...
#head(gtf_data$gene_id) #to check the col details
```
...
```{r}
x <- unique(gtf_ACTB$transcript_id) %>%
  setdiff(NA)
length(x)
...
select(org.Hs.eg.db, head(keys(org.Hs.eg.db)), columns(org.Hs.eg.db))
```
...
```{r}
assay_counts <- read.csv("/cloud/project/data/bioconductor/summarized_experiment/counts.csv", row.names = 1)
assay_counts  
head(assay_counts)
```
...
```{r}
sample_metadata <- read.csv("/cloud/project/data/bioconductor/summarized_experiment/sample_metadata.csv", row.names = 1)
head(sample_metadata)
```

```{r}
gene_metadata <- read.csv("/cloud/project/data/bioconductor/summarized_experiment/gene_metadata.csv", row.names = 1) #1 is the column number
head(gene_metadata)
```
